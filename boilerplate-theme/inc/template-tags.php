 ...
 */
  ...
  */
 function dummy_theme_name_posted_on() {
  ...
  */
 function dummy_theme_name_posted_by() {
 	$byline = sprintf(
 		esc_html( 'by %s' ),
  ...
  */
 function dummy_theme_name_comment_count() {
  ...
  */
 function dummy_theme_name_entry_footer() {
 	// Hide category and tag text for pages.
 	if ( 'post' === get_post_type() ) {
 		/* translators: used between list items, there is a space after the comma */
 		$categories_list = get_the_category_list( esc_html( ', ' ) );
 		if ( $categories_list && dummy_theme_name_categorized_blog() ) {
  ...
  * @return bool
  */
 function dummy_theme_name_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'dummy_theme_name_categories' ) ) ) {
 		...
 		set_transient( 'dummy_theme_name_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so dummy_theme_name_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so dummy_theme_name_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in dummy_theme_name_categorized_blog.
  */
 function dummy_theme_name_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'dummy_theme_name_categories' );
 }
 add_action( 'edit_category', 'dummy_theme_name_category_transient_flusher' );
 add_action( 'save_post',     'dummy_theme_name_category_transient_flusher' );
  ...
  */
 function dummy_theme_name_comment_list( $comment, $args, $depth ) {
  ...
  */
 function dummy_theme_name_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
